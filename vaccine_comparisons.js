...
function colorMap(map,key){
  ...
  //  console.log(colorStart)
    ...
    //
    // console.log(pub.dataByState[pub.currentState])
    // console.log("compare_"+pub.pair)
 ...
 //
    ...
    if(rangeExtent[0]==rangeExtent[1]){
        var color = {property:dataProperty,stops:[[0,"#ddd"]]}
    }
    map.setPaintProperty("counties", 'fill-color', color)
...
function newScatterPlot(state){
    ...
    if(rangeExtent[0]==rangeExtent[1]){
        var cScale = d3.scaleLinear().domain([rangeExtent[0],0,rangeExtent[1]]).range(["#ddd","#ddd","#ddd"])
    }
    ...
    .on("mouseover",function(d){
        ...
        if(d[xLabel]>d[yLabel]){
            var compareString = (d[xLabel]-d[yLabel])+" more doses by "+measureDisplayText[xLabel]+ " than by "+ measureDisplayText[yLabel]
        }else if(d[xLabel]<d[yLabel]){
            var compareString =(d[xLabel]-d[yLabel])+" more doses by "+measureDisplayText[yLabel]+ " than by "+ measureDisplayText[xLabel]
        }else{
            var compareString = measureDisplayText[yLabel]+ " and "+ measureDisplayText[xLabel]+ " result in same number of doses"
        }
        
        var displayString =" <span class=\"popupTitle\">"+ d.county+" County, "+d.stateAbbr+"</span>"+"<br>"
        +"Poplation: "+d["totalPopulation"]+"<br>"
        +measureDisplayText[xLabel]+":  <span class=\"popupTitle\">"+numberWithCommas(d[xLabel])+"</span> doses"+"<br>"
        +measureDisplayText[yLabel]+":  <span class=\"popupTitle\">"+numberWithCommas(d[yLabel])+"</span> doses"+"<br>"
